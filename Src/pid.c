...
#define DT 0.0025f
#define OUTER_DERIV_FILT_ENABLE 0
...
void Double_Roll_Pitch_PID_Calculation(PIDDouble* axis, float set_point_angle, float angle, float rate)
{
	...
	if((axis->out.outer_loop_counter % 2) == 0)
	{
		...
		axis->out.error_sum = axis->out.error_sum + axis->out.error * 0.005;	//Define summation of outer loop
...
#else
	axis->in.error_deriv_filt = axis->in.error_deriv_filt * 0.76f + axis->in.error_deriv * 0.24f;	//filter for derivative
